...
# A transparent space may contain one star, with color/style transitions.
my $gap=qr/(?:\e\[[0-9;]*m)*[ *+.](?:\e\[[0-9;]*m)*/;
sub start_case {
  my ($mode) = @_;
  $pty = IO::Pty->new;
...
start_case('chooser');
expect(qr/Up\/Down:${gap}move/,'initial menu');
...
my $moving_frames=grep {/\e\[\d+;\d+H(?:\e\[0;\d+m)?\e\[38;2;/} @overlay_frames;
...
start_case('static');
expect(qr/Up\/Down:${gap}move/,'static fallback menu');
die 'static mode emitted an animation overlay' if $buf =~ /\e\[\d+;\d+H(?:\e\[0;\d+m)?\e\[38;2;/;
...
start_case('chooser');
expect(qr/Up\/Down:${gap}move/,'menu');
send_keys("\eOA\n");
...
start_case('menu');
expect(qr/Start${gap}Codex/,'dynamic start label');
send_keys("\e[B\n");
...
expect(qr/CREATED:.*?\e\[\?1049h.*?\e\[1mResume${gap}Codex/s,'new session detach returns to selected Resume Codex');
send_keys("\n");
expect(qr/ATTACHED_CODEX.*?\e\[\?1049h.*?\e\[1mResume${gap}Codex/s,'reattach and detach returns to the menu again');
send_keys("\e");
finish();
print "PASS Codex validation, launch, detach to menu, and repeated resume\n";
...
start_case('menu');
expect(qr/Resume${gap}Codex/,'resume label');
send_keys("\e[B\n");
expect(qr/ATTACHED_CODEX/,'resume action');
expect(qr/ATTACHED_CODEX.*?\e\[\?1049h.*?\e\[1mResume${gap}Codex/s,'existing session detach returns to the menu');
send_keys("\e[A\n");
expect(qr/Ready when you are/,'New Terminal still leaves the menu');
finish();
delete $ENV{TEST_SESSION};
print "PASS existing Codex session resume/detach and return to the shell\n";
start_case('chooser');
expect(qr/Up\/Down:${gap}move/,'menu before interrupt');
send_keys("\x03");
...
start_case('chooser');
expect(qr/Ctrl-L:${gap}redraw/,'redraw hint');
